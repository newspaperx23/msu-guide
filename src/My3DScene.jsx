...
function AnimatedModel() {
  ...
  return (
    <primitive
      ref={ref}
      object={cloned}
      scale={window.innerWidth < 768 ? 1.0 : 1.35}
      position={[0, -1.5, 0]}
    />
  );
}
...
function AudioController({
  ...
  const playAudio = async () => {
    ...
    try {
      ...
      const audioUrl =
        selectedPlace.audioUrls[currentLanguage] ||
  ...
  useEffect(() => {
    console.log(
      "Selected place or language changed:",
      selectedPlace?.id,
      currentLanguage
    );
  ...
  if (!selectedPlace || !selectedPlace.audioUrls) {
    console.log(
      "No selected place or no audio URLs, showing only language buttons"
    );
...
export default function My3DScene({
  ...
  const getTypewriterMessages = () => {
    if (
      selectedPlace &&
      audioTranscripts &&
      audioTranscripts[selectedPlace.id]
    ) {
    ...
    if (
      ...
      if (transcript) {
        ...
        const delay = Math.max(
          15,
          Math.min(80, (estimatedDuration * 1000) / totalChars)
        );
  ...
  return (
    <>
      ...
      {/* 3D Scene Container */}
      <div className="fixed bottom-[-10%] right-[5%] md:right-[0%] z-[999] ">
        <div className="relative w-[180px] h-[280px] md:w-[320px] md:h-[420px]">
          {/* กล่องข้อความ - Enhanced with audio integration */}
          <div className="absolute top-[-15%] left-[-10%] md:top-[20%] md:left-[-50%] text-white shadow-lg text-xs md:text-base font-light p-2 md:p-4 rounded-lg md:rounded-xl bg-black/80 backdrop-blur-sm w-[75vw] max-w-[300px] md:w-[240px] z-[9999] border border-white/20 overflow-auto max-h-[35vh] md:max-h-[45vh] break-words whitespace-normal">
            <div key={typewriterKey} className="min-h-[40px] md:min-h-[60px]">
              <Typewriter
                options={{
                  strings: messages,
                  autoStart: true,
